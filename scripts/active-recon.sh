...
cat ../global_output/livehosts.txt | while read ip; do $(cat ../$ip/$ip-exploits.json | sed '/Reading/d' |jq '.RESULTS[].Path' | sed 's/"//g' | while read link; do $(cp $link ../$ip/exploits/);done;); done
